...
export const PalettePicker = () => {
    ...
    const paletteCustom = color ? getPalette(HSLColor) : ''
  ...
  return (
    <>
        <div className="flex gap-5 py-3 items-center">
            ...
            <span className="rounded-full overflow-hidden h-10 w-10 flex items-center justify-center border-black border-2">
        ...
        <section className="grid gap-4 items-center">
            <div className="flex flex-wrap gap-3 text-xs text-black">
                {paletteCustom && paletteCustom.map(item => <div key={item} className="h-32 w-36 flex flex-col-reverse rounded shadow-lg overflow-hidden" style={{background: item}}><span className="bg-white p-2">{item}</span></div>)}
            </div>
            <div className="flex flex-wrap gap-3 text-xs text-black">
